...
The `plotly` package. A godsend for interactive documents, dashboard and presentations. For such documents there is no doubt that anyone would prefer a plot created in `plotly` rather than `ggplot2`. Why? Using `plotly` gives you neat and crucially _interactive_ options at the top, where as `ggplot2` objects are static. In an app we have been devloping here at [Jumping Rivers](https://www.jumpingrivers.com), we found ourselves asking the question would it be quicker to use `plot_ly` or wrapping a `ggplot2` object in `ggplotly`? I found the results staggering.   
