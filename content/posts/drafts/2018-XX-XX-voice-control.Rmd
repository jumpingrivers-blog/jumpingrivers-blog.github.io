...
Annyang works with all browsers that implement the Speech Recognition interface of the Web Speech API and makes it extremely easy to get started. Here is some JavaScript that you can include inside raw HTML to add a voice command that listens for the word "Google" and takes you to the Google home page.
...
As a simple example lets create a counter that increments when I say "go".

Our voice.js file would be
...
server = function(input,output,session){
  output$out = renderText(input$counter)
}
```
