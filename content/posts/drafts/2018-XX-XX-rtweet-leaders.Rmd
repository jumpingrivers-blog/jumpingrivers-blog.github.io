...
The string inside `html_nodes()` is gathered using selector gadget. See this [great tutorial](http://blog.rstudio.com/2014/11/24/rvest-easy-web-scraping-with-r/) on **rvest** and for more on selector gadget read `vignette("selectorgadget")`. Obviously we need to tidy it up a bit. I'm going to skip that bit as it's a tad nasty. The finished product end's up looking like so 
...
```{r, fig.width=10}
leadR_info2 %>% 
ggplot(aes(x = name, y = value,fill = type, colour = type)) +
  geom_col() +
...
Now we plot
```{r, eval = FALSE, fig.width=10, warning=FALSE, message = FALSE}
leadR_twit2 %>% 
